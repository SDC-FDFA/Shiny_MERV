...
## C_2_Government Effectiveness
get_wb_c_2_gov_effectiveness <- function(countries, years) {
...
classify_gov_effectiveness <- function(value) {
  case_when(
    value >= 80 ~ "Very high",
    value < 80 & value >= 60 ~ "High",
    value < 60 & value >= 40 ~ "Medium",
    value < 40 & value >= 20 ~ "Low",
    value < 20 & value >= 0 ~ "Very low",
    TRUE ~ "NA"
  )
}