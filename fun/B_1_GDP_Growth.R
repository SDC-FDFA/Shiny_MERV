...
get_wb_b_1_gni_pc <- function(countries, years) {
  wb_base_url <- "https://data360api.worldbank.org"
  wb_endpoint <- "/data360/data"
  ...
  req_wb <- request(paste0(wb_base_url, wb_endpoint)) |>
    req_url_query(
      DATABASE_ID = "WB_WDI",
      INDICATOR = "WB_WDI_NY_GNP_PCAP_CD",
      REF_AREA = countries,
      #   TIME_PERIOD = years,
      .multi = "comma"
    ) |>
...
gni_pc_label = c("Low Income", "Lower-Middle", "Upper-Middle", "High Income")
gni_pc_min   = c(0,      1136,         4496,      13935)
gni_pc_max   = c(1136,     4496,         13935,      20000)
gni_pc_color = c("#823a53", "#ef7d00", "#60b3b1", "#519795")
