...
output$download_report <- downloadHandler(
  ...
  content = function(file) {
    ...
    # Create and save the plot
    temp_plot_fhfgi <- tempfile(fileext = ".png")

    nyears <- length(unique(df_fhfgi$year))
    main_country <- input$main_country

    ggsave(temp_plot_fhfgi, plot = draw_plot(df_fhfgi, main_country, nyears, "Functioning of Government", "Freedom House"), width = 6, height = 1.8, dpi = 200)

    # Add plot to document
    doc <- doc |>
      body_add_img(src = temp_plot_fhfgi, width = 6, height = 1.8, style = "Compact")

    # Create and save the categories plot
    temp_plot_fhfgi_cat <- tempfile(fileext = ".png")

    main_country <- input$main_country

    ggsave(temp_plot_fhfgi_cat, plot = draw_plot_categories_noval(df_fhfgi, main_country, fhfgi_label, fhfgi_min, fhfgi_max, fhfgi_color), width = 6, height = 0.5, dpi = 200)

    # Add plot to document
    doc <- doc |>
      body_add_img(src = temp_plot_fhfgi_cat, width = 6, height = 0.5, style = "Compact")
    ...
    doc <- doc |>
      body_add_fpar(
        fpar(
          ftext("The categories shown are indicative (cut-off values are not official). The "),
          hyperlink_ftext(
            text = "Functioning of Government",
            href = "https://datafinder.qog.gu.se/variable/icrg_qog",
            prop  = fp_text(color = "#0563C1", underlined = TRUE, font.size = 8)
          ),
          ftext(" is one of three subcategories of Political Rights by Freedom House and ranges from 0 (worst) to 12 (best)."),
          fp_p = fp_par(word_style = "Caption_Note")
        )
      )
